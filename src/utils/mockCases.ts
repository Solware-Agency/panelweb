...
function generateRandomName(): { nombre: string; apellido: string; nombreCompleto: string } {
  const nombres = ['Carlos', 'María', 'José', 'Ana', 'Luis', 'Carmen', 'Roberto', 'Patricia', 'Miguel', 'Laura', 'David', 'Sofia', 'Pedro', 'Isabel', 'Juan', 'Elena']
  const apellidos = ['García', 'Rodríguez', 'Martínez', 'López', 'Hernández', 'González', 'Pérez', 'Sánchez', 'Ramírez', 'Torres', 'Flores', 'Rivera', 'Gómez', 'Díaz', 'Cruz', 'Morales']
...
export function generateMockCases(count: number = 50): MedicalCase[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const case_: MedicalCase = {
      estatus: ['Pendiente', 'En Proceso', 'Completado', 'Cancelado'][Math.floor(Math.random() * 4)] as MedicalCase['estatus'],
      codigo: generateRandomCode(),
      estatusPagoInforme: ['Pendiente', 'Pagado', 'Enviado', 'Completado'][Math.floor(Math.random() * 4)] as MedicalCase['estatusPagoInforme'],
