...
const PatientsList: React.FC = () => {
  ...
  const patients = useMemo(() => {
    ...
    // Group records by id_number
    recordsData.data.forEach(record => {
      if (!record.id_number) return

      const existingPatient = patientMap.get(record.id_number)
      const recordDate = new Date(record.created_at || record.date)

      if (!existingPatient) {
        patientMap.set(record.id_number, {
        ...
        patientMap.set(record.id_number, existingPatient)
      }
    })
        ...
        <div className="md:hidden">
          {sortedPatients.length > 0 ? (
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {sortedPatients.map((patient) => (
                <div key={patient.id_number} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <Card className="p-4">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <p className="text-sm text-gray-500 dark:text-gray-400">Pacientes Encontrados</p>
