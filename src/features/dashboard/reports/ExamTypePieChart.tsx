...
const ExamTypePieChart: React.FC = () => {
  ...
  const pieData = stats?.revenueByExamType?.map((item, index) => ({
    ...item,
    percentage: totalRevenue > 0 ? (item.revenue / totalRevenue) * 100 : 0,
    color: getExamTypeColor(index),
    startAngle: index === 0 ? 0 : stats.revenueByExamType.slice(0, index).reduce((sum, i) => 
      sum + (totalRevenue > 0 ? (i.revenue / totalRevenue) * 360 : 0), 0),
    endAngle: stats.revenueByExamType.slice(0, index + 1).reduce((sum, i) => 
      sum + (totalRevenue > 0 ? (i.revenue / totalRevenue) * 360 : 0), 0)
  })) || [];
            ...
            {isLoading ? (
              ...
              <div className="relative">
                ...
                {hoveredSegmentIndex !== null && (
                  <div 
                    className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-full bg-white dark:bg-gray-800 p-2 rounded shadow-lg border border-gray-200 dark:border-gray-700 z-10 min-w-[150px] text-center"
                    style={{
                      opacity: hoveredSegmentIndex !== null ? 1 : 0,
                      transition: 'opacity 0.2s ease'
                    }}
                  >
                    <p className="font-medium text-gray-900 dark:text-gray-100">
                      {pieData[hoveredSegmentIndex].examType}
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400">
                      {formatCurrency(pieData[hoveredSegmentIndex].revenue)}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-500">
                      {pieData[hoveredSegmentIndex].percentage.toFixed(1)}% del total
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2">
                {pieData.map((item, index) => (
                  <div 
                    ...
                    onMouseLeave={() => setHoveredSegmentIndex(null)}
                    style={{
                      transform: hoveredSegmentIndex === index ? 'scale(1.02)' : 'scale(1)',
                    }}
                  >
