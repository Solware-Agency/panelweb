-- Fix the log_medical_record_deletion function to use correct field names
-- The function was trying to access OLD.full_name which doesn't exist in medical_records_clean
...
BEGIN
  ...
  -- Create record info string using available fields (fixed: removed OLD.full_name)
  record_info := COALESCE(OLD.code, 'Sin código') || ' - ' || COALESCE(OLD.exam_type, 'Sin tipo de examen');
  
  -- Save the deletion log (using only patient_id to avoid FK constraint issues)
