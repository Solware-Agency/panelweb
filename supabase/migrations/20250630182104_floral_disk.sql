/*
  ...
  1. New Tables
    - `profiles`
      ...
      - `assigned_branch` (text, nullable, for employee branch assignment)
...
-- Add constraints
ALTER TABLE profiles ADD CONSTRAINT IF NOT EXISTS profiles_role_check 
  CHECK (role = ANY (ARRAY['owner'::text, 'employee'::text]));

ALTER TABLE profiles ADD CONSTRAINT IF NOT EXISTS profiles_estado_check 
